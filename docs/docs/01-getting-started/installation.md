...
### Desktop GUI App (`.dmg`)
1. Download `Membuss-v2.9.2-darwin-arm64.dmg` (for M1/M2/M3/M4/M5 Macs) or `Membuss-v2.9.2-darwin-amd64.dmg` (for Intel Macs).
...
```bash
# Apple Silicon (M1–M5)
curl -fsSL https://github.com/membuss-protocol/membuss/releases/download/v2.9.2/membuss-v2.9.2-darwin-arm64.tar.gz | tar -xz -C /usr/local/bin

# Intel Macs
curl -fsSL https://github.com/membuss-protocol/membuss/releases/download/v2.9.2/membuss-v2.9.2-darwin-amd64.tar.gz | tar -xz -C /usr/local/bin
...
1. Download `Membuss-v2.9.2-windows-amd64-installer.exe`.
2. Double-click the installer and follow the setup wizard.
3. Launch **Membuss Desktop** from your Start Menu or Desktop shortcut.

### Portable Binary (`.zip`)
1. Download `membuss-v2.9.2-windows-amd64.zip`.
...
```bash
# Download AppImage
wget https://github.com/membuss-protocol/membuss/releases/download/v2.9.2/Membuss-v2.9.2-linux-amd64.AppImage

# Make executable
chmod +x Membuss-v2.9.2-linux-amd64.AppImage

# Run Desktop App
./Membuss-v2.9.2-linux-amd64.AppImage
```

### Headless Server Archive (`.tar.gz`)
```bash
# Download for Linux x64
wget https://github.com/membuss-protocol/membuss/releases/download/v2.9.2/membuss-v2.9.2-linux-amd64.tar.gz

# Extract binary to /usr/local/bin
sudo tar -C /usr/local/bin -xzf membuss-v2.9.2-linux-amd64.tar.gz
